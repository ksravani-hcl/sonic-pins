...
TrafficMetrics GetTrafficMetrics(const otg::FlowMetric& flow_metric,
  ...
  const absl::Duration total_duration = [=, &flow_metric] {
    ...
    if (flow_rate.choice() == otg::FlowRate::Choice::pps) {
      ...
      // TODO: samueldong - Evaluate either improving this logic or recommending
      // packets per seconds for timing tests (currently this is only used to
      // validate 0 loss which will still work). This actually isn't accurate
      // because the data rate is lower than linerate due to overhead, so this
      // will underestimate the true loss. PPS is probably better for accurate
      // timing, though with investigation and knowing the packet size could
